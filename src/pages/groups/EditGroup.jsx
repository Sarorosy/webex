...
const EditGroup = ({ selectedGroup, onClose , finalFunction }) => {
  ...
  useEffect(() => {
      ...
      const fetchInfo = async () => {
        try {
          const res = await fetch(`https://webexback-vb1k.onrender.com/api/groups/group/${selectedGroup.group_id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setCreating(true)
      const response = await fetch('https://webexback-vb1k.onrender.com/api/groups/update', {
        method: 'POST',
        headers: {
